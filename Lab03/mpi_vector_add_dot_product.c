/* File:     mpi_vector_add_dot_product.c
 *
 * Purpose:  Implement parallel dot product of two vectors using
 *           MPI. This version uses a block distribution.
 *
 * Compile:  mpicc -g -Wall -o mpi_vector_add_dot_product mpi_vector_add_dot_product.c
 * Run:      mpiexec -n <comm_sz> ./vector_add_dot_product
 *
 * Input:    The order of the vectors, n, and the vectors x and y
 * Output:   x and y vectors multiplied by a scalar and the dot product
...
void Parallel_vector_scalar_multiplication(double local_x[], double local_y[],
      int local_n, int n);
double Parallel_vector_product( double local_x[], double local_y[],
      int local_n);
...
int main(void) {
   ...
   Print_vector(local_x, local_n, n, "\nFirst vector is", my_rank, comm);
   Read_vector(local_y, local_n, n, "y", my_rank, comm);
   Print_vector(local_y, local_n, n, "\nSecond vector is", my_rank, comm);

   Parallel_vector_scalar_multiplication(local_x, local_y, local_n, n);

   Print_vector(local_x, local_n, n, "\nFirst vector multiplied with scalar is", my_rank, comm);

   Print_vector(local_y, local_n, n, "\nSecond vector multiplied with scalar is", my_rank, comm);

   double dot_product = Parallel_vector_product(local_x, local_y, local_n);

   printf("\nProducto punto es igual a %f\n", dot_product);
...
/*-------------------------------------------------------------------
 * Function:  Parallel_vector_scalar_multiplication
 * Purpose:   Multiplies the two vectors by a scalar
 * In args:   n:  scalar to multiply the vectors by
 *            local_n:  the number of components in local_x, local_y
 * Out arg:   local_x:  local storage of one of the vectors being added
 *            local_y:  local storage for the second vector being added
 */
void Parallel_vector_scalar_multiplication(
    ...
    for (local_i = 0; local_i < local_n; local_i++) {
        local_x[local_i] = local_x[local_i] * n;
        local_y[local_i] = local_y[local_i] * n;
    }
} /* Parallel_vector_scalar_multiplication */

/*-------------------------------------------------------------------
 * Function:  Parallel_vector_product
 * Purpose:   Calculates the dot product of two vectors
 * In args:   n:  scalar to multiply the vectors by
 *            local_x:  local storage of one of the vectors being added
 *            local_y:  local storage for the second vector being added
 * Return:    The dot product of the two vectors
 */
double Parallel_vector_product(
      double  local_x[]  /* in  */,
      double  local_y[]  /* in  */,
      int     local_n    /* in  */) {
